...
if cx is not None and len(cx):
    sub("한국 화장품 수출 — 관세청",
        "연도 × 국가 × HS(3303~3307) · US$ · 2026년은 1~5월 누계(잠정)")
    ...
    k2.metric(f"{_y_full}년 1위 시장: {_top_c.index[0]}",
              ...
              f"점유 {_top_c.iloc[0] / _top_c.sum() * 100:.0f}%",
              delta_color="off")
    k3.metric(f"{int(cx['year'].max())}년 1~5월 누계",
              f"${_ytot[cx['year'].max()]:,.2f}bn",
    ...
    with x1:
        sub("연도별 수출 — 권역 구성", "US$mn · 막대 위 = 연 합계(US$bn)")
    ...
    with x2:
        ...
        st.caption("**읽는법**: 거의 모든 나라에서 3304(기초·색조)가 대부분. "
                   "선케어는 HS상 분리 불가(3304 안에 포함), 마스크팩은 "
                   "3307에 섞임 — 관세청 분류의 한계.")

    # 톱 10 국가 표
    _yy = _y_full
    t_now = cx[cx["year"] == _yy].groupby(
        ["country_en", "region"], as_index=False)["usd_k"].sum()
    t_prev = cx[cx["year"] == _yy - 1].groupby(
    ...
    view_t.columns = ["국가", "권역", f"{_yy} 수출(US$mn)", "YoY", "점유율"]
    _num2 = st.column_config.NumberColumn
    st.dataframe(view_t, hide_index=True, use_container_width=True,
                 column_config={f"{_yy} 수출(US$mn)": _num2(format="localized"),
                                "YoY": _num2(format="%+.0f%%"),
                                "점유율": _num2(format="%.1f%%")})
    st.caption("**출처**: 관세청 무역통계 (실리콘투 워크북 Exports_RawData에 정리된 "
               "것을 추출 — customs_exports_sync.py). 갱신: 워크북 갱신 후 스크립트 "
               "재실행. 월간 자동화는 data.go.kr OpenAPI 키 발급(사용자 신청 필요) "
               "후 kbeauty 파이프라인으로 전환 예정.")
